...
def predict_latest_season(df):
    ...
    reg_seasons = sorted(df[df['season'].str.contains('-reg')]['season'].unique())
    latest_season = reg_seasons[-1]

    completed_seasons = [
        s for s in seasons
        if historical[historical["season"] == s]["deep_playoff_run"].nunique() == 2
    ]

    print("\n=== Leave-One-Season-Out Validation ===")
    loso_results = []

    for held_out in completed_seasons:
    ...
    print(f"Predicting Season: {latest_season}")

    train_df  = df[df['season'] != latest_season].dropna(subset=TEAM_FEATURES + ["deep_playoff_run"])
    pred_df = df[df['season'] == latest_season].dropna(subset=TEAM_FEATURES)


    X_train = train_df[TEAM_FEATURES]
    y_train = train_df["deep_playoff_run"]

    X_pred = pred_df[TEAM_FEATURES]
    ...
    results = pred_df[["team_name"]].copy()
    ...
    print("\nPredicted Conference Final Teams (Top4):\n")
...
def main():
    ...
    latest_reg_season = reg_df["season"].max()
    latest_reg_label = f"{latest_reg_season}-reg"


    # Build latest regular sesason rows
    latest_reg = reg_df[reg_df["season"] == reg_df["season"].max()].copy()
    latest_reg["playoff_stage"] = "Unknown"   
    latest_reg["deep_playoff_run"] = 0
    latest_reg["season"] = latest_reg_label    
    ...
    latest_reg_top1["season"] = latest_reg_label
